...
func (b *tfeAuthBackend) pathsRole() []*framework.Path {
	p := []*framework.Path{
		{
			Pattern: "role/?",
...
func (b *tfeAuthBackend) pathRoleExistenceCheck(ctx context.Context, req *logical.Request, data *framework.FieldData) (bool, error) {
	b.l.RLock()
	defer b.l.RUnlock()
...
func (b *tfeAuthBackend) pathRoleList(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	b.l.RLock()
	defer b.l.RUnlock()
...
func (b *tfeAuthBackend) pathRoleRead(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func (b *tfeAuthBackend) pathRoleDelete(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	roleName := data.Get("name").(string)
	if roleName == "" {
...
// of an existing role
func (b *tfeAuthBackend) pathRoleCreateUpdate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	roleName := data.Get("name").(string)
	if roleName == "" {
		return logical.ErrorResponse("missing role name"), nil
	}
	...
	if workspaces, ok := data.GetOk("workspaces"); ok {
		role.Workspaces = workspaces.([]string)
	} else if req.Operation == logical.CreateOperation {
		role.Workspaces = data.Get("workspaces").([]string)
	}
	// Verify namespaces is not empty
	if len(role.Workspaces) == 0 {
		return logical.ErrorResponse("\"workspaces\" can not be empty"), nil
	}
	// Verify * was not set with other data
	if len(role.Workspaces) > 1 && strutil.StrListContains(role.Workspaces, "*") {
...
type roleStorageEntry struct {
	...
	// Workspaces is the array of workspaces able to access this
	// role.
	Workspaces []string `json:"workspaces" mapstructure:"workspaces" structs:"workspaces"`
